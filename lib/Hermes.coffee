...
getViews = (viewRoot, name) ->
  ...
  views.text = if path.existsSync p = getPath '' then getView p
  else if path.existsSync p = getPath '.text' then getView p

  views.html = getView p if path.existsSync p = getPath '.html'    
